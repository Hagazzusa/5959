...
def read_squad_examples(input_file, is_training, version_2_with_negative, use_eda):
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                if use_eda > 0 and is_training:
                    aug_question = EDA(question_text)
                    for i in range(min([len(aug_question), use_eda])):
                        example = SquadExample(
                            qas_id=qas_id,
                            question_text=aug_question[i],
                            doc_tokens=doc_tokens,
                            orig_answer_text=orig_answer_text,
                            start_position=start_position,
                            end_position=end_position,
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    logger.warning("answer start, end pos == 0 cnt %d"%err_cnt)
...
use_lamb = True #@param {type: "boolean"}
use_eda =  1#@param {type: "integer"}
...
model_config = "src/bert_small.json"
vocab_file = "src/ko_vocab_32k.txt"
# Prepare model
...
output_prediction_file = sys.argv[2]
